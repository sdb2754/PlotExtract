...
public class Figure extends ZoomPane {
	...
	*/
	...
	private void scaleimage() {
		...
		// If aspect ratio of image > aspect ratio of plot area
		// Scale width of image
		// Else scale heigh of image
	...
	// Distance between a point and a pair of integer coordinates
		...
		if (distance(origin, x, y) < 10) {
			...
			if (isright) {
				s = (String) JOptionPane.showInputDialog(Board.frame,
						"What would you like to do to the Origin point?:\n", "Select Origin", JOptionPane.PLAIN_MESSAGE,
						null, possibilities, "move");
				switch (s) {
				case "move":
					move = true;
					break;
		...
		if (distance(x_cal, x, y) < 10) {
			...
			if (isright) {
				...
				s = (String) JOptionPane.showInputDialog(Board.frame,
						...
						JOptionPane.PLAIN_MESSAGE, null, possibilities, "move");
				switch (s) {
				case "move":
					move = true;
					break;
		...
		for (int i = 0; i < data.size(); i++)
			if (distance(data.get(i), x, y) < 10) {
				...
				if (isright) {
					...
					s = (String) JOptionPane.showInputDialog(Board.frame, "What would you like to do to this data?:\n",
							"Data point", JOptionPane.PLAIN_MESSAGE, null, possibilities, "move");
					switch (s) {
					case "export":
						export();
						break;
	...
	private void disp_coordinate(Point p) {
		JOptionPane.showMessageDialog(Board.frame, "Point coordinate: (" + get_x(p.x) + "," + get_y(p.y) + ")");
	}

	private void disp_formula() {
		JOptionPane.showMessageDialog(Board.frame, "The formula of the fit is: " + formula);
	}

	private void choose_fit() {
		String s;
		Object[] fit_types = { "linear", "connect", "spline", "interp", "regression", "none" };
		s = (String) JOptionPane.showInputDialog(Board.frame, "Select fit type:\n", "Fit type",
				JOptionPane.PLAIN_MESSAGE, null, fit_types, "linear");
		set_fit(s);
	}
	...
	private void set_type_x() {
		...
		String s;
		s = (String) JOptionPane.showInputDialog(Board.frame, "What type is the Horizontal axis?:\n", "Set X axis type",
				JOptionPane.PLAIN_MESSAGE, null, types, "linear");
		if (s == "log") {
			x_log = true;
		} else {
			x_log = false;
	...
	private void set_type_y() {
		Object[] types = { "log", "linear" };
		String s;
		s = (String) JOptionPane.showInputDialog(Board.frame, "What type is the Vertical axis?:\n", "Set Y axis type",
				JOptionPane.PLAIN_MESSAGE, null, types, "linear");
		if (s == "log") {
			y_log = true;
		} else {
	...
	public void set_fit(String s) {
		if (s == "regression") {
			try {
				order = Integer.parseInt(JOptionPane.showInputDialog(Board.frame, "Enter regression order: ", null));
				if (order < 1)
					order = 1;
	...
	public void update_table() {
		...
		// X and Y reference and calibration points
		if (step > 2) {
	...
	private void set_origin_location() {
		int iszero = JOptionPane.showConfirmDialog(null, "Is the origin at (0,0)?", null, JOptionPane.YES_NO_OPTION);
		if (iszero != JOptionPane.YES_OPTION) {
			try {
				x_ori = Integer.parseInt(JOptionPane.showInputDialog(Board.frame, "Enter origin X value: ", null));
			} catch (RuntimeException e) {
				x_ori = origin.x;
			}
			try {
				y_ori = Integer.parseInt(JOptionPane.showInputDialog(Board.frame, "Enter origin Y value: ", null));
			} catch (RuntimeException e) {
				y_ori = origin.y;
			}
		}
	...
	private void set_x_ref() {
		try {
			x_ref = Integer.parseInt(JOptionPane.showInputDialog(Board.frame, "Enter known X value: ", null));
		} catch (RuntimeException e) {
			x_ref = x_cal.x;
		}
	}
	...
	private void set_y_ref() {
		try {
			y_ref = Integer.parseInt(JOptionPane.showInputDialog(Board.frame, "Enter known Y value: ", null));
		} catch (RuntimeException e) {
			y_ref = y_cal.y;
		}
	}
	...
	public void set_step(int i) {
		...
		case 4:
			data.clear();
			messages.setText(
					"Click to add data points along the curve of interest…Then click ‘Fit’ to select curve fit.");
			break;
		}
		step = i;
		move = false;
	...
	private void linear_fit(int dx) {
		...
		formula = "formula not yet available for a linear fit.";
	...
	private void spline_fit(int dx) {
		...
		for (int x = xa; x < xb; x += dx) {
			...
			fitdata.add(
					new Point(x, (int) (data.get(pc).y + D.get(pc, 0) * t + c * Math.pow(t, 2) + d * Math.pow(t, 3))));
		}
	}
	...
	private void connect_fit(int dx) {
		...
		for (int x = xa; x < xb; x += dx) {
			...
			m = (data.get(pc + 1).y - data.get(pc).y) / (data.get(pc + 1).x - data.get(pc).x);
			b = data.get(0).y - m * data.get(0).x;
			fitdata.add(new Point(x, (int) (m * x + b)));
		}
	}
	...
	private int get_npoints() {
		try {
			npoints = Integer.parseInt(
					JOptionPane.showInputDialog(Board.frame, "Enter number of interpolation points to export: ", null));
		} catch (RuntimeException e) {
			npoints = data.size();
		}
	...
	void get_headers() {
		...
		try {
			x_header = JOptionPane.showInputDialog(Board.frame, "Enter Horizontal Axis label: ", null);
		} catch (RuntimeException e) {
			x_header = "Abscissa";
		}
		try {
			y_header = JOptionPane.showInputDialog(Board.frame, "Enter Vertical Axis label: ", null);
		} catch (RuntimeException e) {
			y_header = "Ordinate";
		}
	}
